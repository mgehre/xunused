...
public:
  ...
  void handleNewOrDeleteUse(const FunctionDecl *decl, const MatchFinder::MatchResult &result) {
      // is not implicitly generated by a compiler and does not belong to a system header
      if (!decl->isImplicit() && !result.SourceManager->isInSystemHeader(decl->getBeginLoc())) {
    ...
    if (const auto *F = Result.Nodes.getNodeAs<FunctionDecl>("fnDecl")) {
      ...
      handleUse(R->getConstructor(), Result.SourceManager);
        ...
        if (const auto opNew = R->getOperatorNew()) {
            handleNewOrDeleteUse(opNew, Result);
        }
        if (const auto opDelete = R->getOperatorDelete()) {
            handleNewOrDeleteUse(opDelete, Result);
        }
